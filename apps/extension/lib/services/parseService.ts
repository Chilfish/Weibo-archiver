...
export class PostParser {
  static parse(rawPost: RawPost): Post | undefined {
    ...
    const hasRetweeted
      = 'retweeted_status' in rawPost && rawPost.retweeted_status?.id
    if (hasRetweeted) {
      retweeted_status = PostParser.parseRetweet(
        rawPost.retweeted_status,
        rawPost.url_struct || [],
      )
    }

    const meta = PostParser.parseMeta(rawPost)
    const text = PostParser.parseText(
      rawPost.text_raw,
      rawPost.url_struct || [],
    )
    ...
    const { reposts_count, comments_count, attitudes_count, is_show_bulletin }
      = rawPost
  ...
  static parseMeta(rawPost: RawPost | RawRetweetedStatus): PostMeta {
    const { id, region_name, created_at, source } = rawPost
  ...
  static parseText(rawText: string, urlStruct: RawUrlStruct[]): string {
    ...
    const replacedText = rawText.replace(r, (match) => {
      ...
      if (longUrl?.pic_infos) {
        const imgsUrl
          = Object.values(longUrl.pic_infos)[0]?.woriginal.url || match
        return `[img://${imgsUrl}]`
      }
  ...
  static parseLinkCard(rawPost: RawPost): LinkCard | undefined {
    ...
    const desc
      = rawPageInfo.page_desc || rawPageInfo.content1 || rawPageInfo.content2
  ...
  static parseBookmark(
    rawBookmark: RawFavorite,
    favBy: string,
  ): Favorite | undefined {
    ...
    let retweeted_status: Retweet | undefined
    const hasRetweeted
      = 'retweeted_status' in rawBookmark && rawBookmark.retweeted_status?.id
    if (hasRetweeted) {
      retweeted_status = PostParser.parseRetweet(
        rawBookmark.retweeted_status,
        rawBookmark.url_struct || [],
      )
    }

    const meta = PostParser.parseMeta(rawBookmark)
    const text = PostParser.parseText(
      rawBookmark.text_raw,
      rawBookmark.url_struct || [],
    )
    ...
    const { reposts_count, comments_count, attitudes_count } = rawBookmark
