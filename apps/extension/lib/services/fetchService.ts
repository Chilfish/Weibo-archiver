...
export class FetchService {
  ...
  async userFollowings(
    args: FetchArgs['userFollowings'],
  ): Promise<RawFollowings> {
    const { data } = await this.fetcher<
      RawFollowings,
      FetchArgs['userFollowings']
    >(FETCH_PATH.FOLLOWINGS, args)
  ...
  async myFollowings(
    args: FetchArgs['myFollowings'],
  ): Promise<RawMyFollowings['follows']> {
    const { data } = await this.fetcher<
      RawMyFollowings,
      FetchArgs['myFollowings']
    >(FETCH_PATH.FOLLOWINGS_MINE, args)
  ...
  async comments(args: FetchArgs['postComments']): Promise<RawComment[]> {
    const { data } = await this.fetcher<
      RawComments['data'],
      FetchArgs['postComments']
    >(FETCH_PATH.POST_COMMENTS, args)
