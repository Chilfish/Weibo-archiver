...
安装油猴脚本：[weibo-archiver.user.js][releases]（如果下载缓慢可用境内加速的 [镜像地址]）。在登录后的[微博网页版]刷新后将启动脚本，点击开始后将开始获取数据。支持断点续传，可恢复到上次的进度。
...
同时也支持 ClI 模式，可以在 [Node.js 官网] 下载安装 Node.js 后，使用 `pnpm dlxx weibo-archiver@latest` 来启动，具体的使用方法可见 [README](apps/cli/README.md)
...
- 为了减轻微博服务器的压力，或是避免一些不必要的限制，请尽量在比较人少的时间段使用
...
- 评论区：获取时，将默认同时获取前 5 条热评，总数不超过 20 条。这是为了避免频繁调用接口，而可能会导致的不明问题。暂时也不支持楼中楼
